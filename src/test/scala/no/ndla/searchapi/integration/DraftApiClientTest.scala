  ...
  test("that dumping articles returns articles in serializable format") {
    ...
    forgePact
      ...
      .runConsumerTest { mockConfig =>
        ...
        val articleApiClient = new DraftApiClient(mockConfig.baseUrl)
        val chunks = articleApiClient.getChunks[domain.draft.Draft].toList
        val draftToSerialize = chunks.flatMap(_.get).head
      // TODO: test at de serialiseres på en fornuftig måte i guess
