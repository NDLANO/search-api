  ...
  test("That /article/ returns 200 ok") {
    ...
    when(
      articleSearchService.all(any[List[Long]],
                               ...
                               any[Sort.Value],
                               any[Seq[String]],
                               any[Boolean])).thenReturn(Success(articleResult))
  ...
  test("That /learningpath/ returns 200 ok") {
    ...
    when(
      learningPathSearchService
        .all(any[List[Long]], any[Option[String]], any[String], any[Sort.Value], any[Int], any[Int], any[Boolean]))
      .thenReturn(Success(lpResult))
