...
class DraftSearchServiceTest extends UnitSuite with TestEnvironment {
  ...
  test(
    ...
    draftSearchService.getStartAtAndNumResults(page, SearchApiProperties.DefaultPageSize) should equal(
      (expectedStartAt, SearchApiProperties.DefaultPageSize))
  }
  ...
  test("all should return only articles of a given type if a type filter is specified") {
    val Success(results) = draftSearchService.all(List(),
                                                  Language.DefaultLanguage,
                                                  None,
                                                  1,
                                                  10,
                                                  Sort.ByIdAsc,
                                                  Seq(LearningResourceType.TopicArticle.toString),
                                                  fallback = false)
    results.totalCount should be(3)
    results.results.map(_.id) should be(Seq(8, 9, 11))

    val Success(results2) = draftSearchService.all(List(),
                                                   Language.DefaultLanguage,
                                                   None,
                                                   1,
                                                   10,
                                                   Sort.ByIdAsc,
                                                   LearningResourceType.all,
                                                   fallback = false)
  ...
  test("That all returns all documents ordered by id ascending") {
    val Success(results) =
      draftSearchService.all(List(), Language.DefaultLanguage, None, 1, 10, Sort.ByIdAsc, Seq.empty, fallback = false)
  ...
  test("That all returns all documents ordered by id descending") {
    val Success(results) =
      draftSearchService.all(List(), Language.DefaultLanguage, None, 1, 10, Sort.ByIdDesc, Seq.empty, fallback = false)
    val hits = results.results
    results.totalCount should be(9)
  ...
  test("That all filtered by id only returns documents with the given ids") {
    val Success(results) = draftSearchService.all(List(1, 3),
                                                  Language.DefaultLanguage,
                                                  None,
                                                  1,
                                                  10,
  ...
  test("That paging returns only hits on current page and not more than page-size") {
    val Success(page1) =
      draftSearchService.all(List(), Language.DefaultLanguage, None, 1, 2, Sort.ByTitleAsc, Seq.empty, fallback = false)
    ...
    val Success(page2) =
      draftSearchService.all(List(), Language.DefaultLanguage, None, 2, 2, Sort.ByTitleAsc, Seq.empty, fallback = false)
  ...
  test("mathcingQuery should filter results based on an article type filter") {
    val Success(results) = draftSearchService.matchingQuery("bil",
                                                            List(),
                                                            "nb",
                                                            None,
                                                            1,
    ...
    val Success(results2) = draftSearchService.matchingQuery("bil",
                                                             List(),
                                                             "nb",
                                                             None,
                                                             1,
                                                             10,
                                                             Sort.ByRelevanceDesc,
                                                             Seq(LearningResourceType.Article.toString),
                                                             fallback = false)
  ...
  test("That search matches title") {
    val Success(results) = draftSearchService.matchingQuery("Pingvinen",
                                                            List(),
                                                            "nb",
                                                            None,
                                                            1,
  ...
  test("That search matches tags") {
    val Success(results) =
      draftSearchService.matchingQuery("and", List(), "nb", None, 1, 10, Sort.ByTitleAsc, Seq.empty, fallback = false)
    val hits = results.results
    results.totalCount should be(1)
    hits.head.id should be(3)
  }
  ...
  test("That search does not return superman since it has license copyrighted and license is not specified") {
    val Success(results) = draftSearchService.matchingQuery("supermann",
                                                            List(),
                                                            "nb",
                                                            None,
                                                            1,
  ...
  test("Searching with logical AND only returns results with all terms") {
    val Success(search1) = draftSearchService.matchingQuery("bilde + bil",
                                                            List(),
                                                            "nb",
                                                            None,
                                                            1,
    ...
    val Success(search2) = draftSearchService.matchingQuery("batmen + bil",
                                                            List(),
                                                            "nb",
                                                            None,
                                                            1,
    ...
    val Success(search3) = draftSearchService.matchingQuery("bil + bilde - flaggermusmann",
                                                            List(),
                                                            "nb",
                                                            None,
                                                            1,
    ...
    val Success(search4) = draftSearchService.matchingQuery("bil - hulken",
                                                            List(),
                                                            "nb",
                                                            None,
                                                            1,
  ...
  test("search in content should be ranked lower than introduction and title") {
    val Success(search) = draftSearchService.matchingQuery("mareritt + ragnarok",
                                                           List(),
                                                           "nb",
                                                           None,
                                                           1,
  ...
  test("searching for notes should return relevant results") {
    val Success(search) = draftSearchService.matchingQuery("kakemonster",
                                                           List(),
                                                           "nb",
                                                           None,
                                                           1,
  ...
  test("Search for all languages should return all articles in correct language") {
    val Success(search) =
      draftSearchService.all(List(), Language.AllLanguages, None, 1, 100, Sort.ByIdAsc, Seq.empty, fallback = false)
    val hits = search.results
  ...
  test("Search for all languages should return all languages if copyrighted") {
    val Success(search) = draftSearchService.all(List(),
  ...
  test("Searching with query for all languages should return language that matched") {
    val Success(searchEn) = draftSearchService.matchingQuery("Big",
                                                             List(),
                                                             "all",
                                                             None,
                                                             1,
                                                             ...
                                                             Seq.empty,
                                                             fallback = false)
    val Success(searchNb) = draftSearchService.matchingQuery("Store",
                                                             List(),
                                                             "all",
                                                             None,
                                                             1,
  ...
  test("That searching with fallback parameter returns article in language priority even if doesnt match on language") {
    val Success(search) =
      draftSearchService.all(List(9, 10, 11), "en", None, 1, 10, Sort.ByIdAsc, Seq.empty, fallback = true)
